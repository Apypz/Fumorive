...
async def change_password(
    ...
    if current_user.oauth_provider:
        raise HTTPException(
            ...
            detail=f"Akun ini menggunakan {current_user.oauth_provider.capitalize()} OAuth. Ganti password melalui platform tersebut."
        )
    ...
    if not user or not user.hashed_password:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Akun tidak memiliki password yang bisa diubah."
        )
    ...
    # Invalidate cache so tokens re-validate
